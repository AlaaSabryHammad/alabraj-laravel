        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            <form action="{{ route('projects.update', $project) }}" method="POST" enctype="multipart/form-data"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input type="number" id="bank_guarantee_amount" name="bank_guarantee_amount"
                                value="{{ old('bank_guarantee_amount', $project->bank_guarantee_amount) }}" step="0.01"
                                min="0"
                        ...
                        <div>
                            ...
                            <select id="bank_guarantee_type" name="bank_guarantee_type"
                                ...
                                <option value="cash"
                                    {{ old('bank_guarantee_type', $project->bank_guarantee_type) == 'cash' ? 'selected' : '' }}>
                                    كاش</option>
                                <option value="facilities"
                                    {{ old('bank_guarantee_type', $project->bank_guarantee_type) == 'facilities' ? 'selected' : '' }}>
                                    تسهيلات</option>
                            </select>
                        ...
                        <div>
                            <label for="client_name" class="block text-sm font-medium text-gray-700 mb-2">اسم
                                العميل</label>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label for="status" class="block text-sm font-medium text-gray-700 mb-2">حالة المشروع
                                *</label>
                            <select id="status" name="status"
                                ...
                                <option value="on_hold"
                                    {{ old('status', $project->status) === 'on_hold' ? 'selected' : '' }}>متوقف مؤقتاً
                                </option>
                        ...
                        <div>
                            <label for="progress" class="block text-sm font-medium text-gray-700 mb-2">نسبة الإنجاز
                                (%)</label>
                        ...
                        <div>
                            <label for="start_date" class="block text-sm font-medium text-gray-700 mb-2">تاريخ البداية
                                *</label>
                        ...
                        <div>
                            <label for="end_date" class="block text-sm font-medium text-gray-700 mb-2">تاريخ النهاية
                                المتوقع</label>
                        ...
                        <div>
                            <label for="project_manager_id" class="block text-sm font-medium text-gray-700 mb-2">مدير
                                المشروع</label>
                            <select id="project_manager_id" name="project_manager_id"
                                ...
                                @foreach (\App\Models\Employee::where('status', 'active')->orderBy('name')->get() as $employee)
                                    <option value="{{ $employee->id }}"
                                        ...
                                        {{ $employee->name }} @if ($employee->position)
                                            - {{ $employee->position }}
                                        @endif
                                    </option>
                                @endforeach
                            ...
                            <input type="hidden" name="project_manager"
                                value="{{ old('project_manager', $project->project_manager) }}">
                        </div>

                        <div>
                            <label for="location" class="block text-sm font-medium text-gray-700 mb-2">موقع المشروع
                                *</label>
                ...
                <div>
                                ...
                                @foreach ($project->projectImages as $image)
                                    <div class="relative group">
                                        ...
                                        <div class="absolute top-2 right-2">
                                            <button type="button" onclick="deleteProjectImage({{ $image->id }})"
                    ...
                    <!-- Add New Images -->
                    <div>
                        <label for="new_images" class="block text-sm font-medium text-gray-700 mb-2">إضافة صور
                            جديدة</label>
                        ...
                        @error('new_images.*')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                        <p class="text-sm text-gray-500 mt-1">يمكنك رفع عدة صور (PNG, JPG, JPEG, GIF) - الحد الأقصى لحجم
                            الصورة 2 ميجابايت</p>
                    </div>
                </div>
    ...
    <script>
        ...
        function showNotification(message, type = 'info') {
            ...
            notification.className =
                `fixed top-4 right-4 ${colors[type]} text-white px-6 py-3 rounded-lg shadow-lg z-50 transition-all duration-300`;
