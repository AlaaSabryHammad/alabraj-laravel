...
@section('title', 'تعديل المستخلص - ' . $extract->extract_number)
...
@endphp
    <div class="p-6" dir="rtl">
        ...
        <div class="bg-white rounded-xl shadow-sm border p-6 mb-6">
            <div class="flex items-center justify-between mb-4">
                <div>
                    <h1 class="text-2xl font-bold text-gray-900">تعديل مستخلص مشروع</h1>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                ...
                <div>
                    <span class="font-medium text-gray-700">تاريخ التعديل:</span>
        ...
        @if ($project->projectItems && $project->projectItems->count() > 0)
            ...
            @endphp
            <form id="extractForm" action="{{ route('projects.extract.update', [$project, $extract]) }}" method="POST"
                ...
                <div class="bg-white rounded-xl shadow-sm border overflow-hidden">
                            ...
                            <tbody>
                                ...
                                @foreach ($project->projectItems as $index => $item)
                                    @php
                                        ...
                                        $currentQuantity = $currentQuantities[$index] ?? 0;

                                        // تعيين كمية المستخلص الحالي وقيمته
                                    ...
                                    <tr class="border-b">
                                        ...
                                        <td class="px-4 py-3 border text-center bg-blue-50">
                                            ...
                                            <input type="number" step="0.01" min="0" max="{{ $maxQuantity }}"
                                                value="{{ $currentQuantity }}" placeholder="أدخل الكمية"
                                        ...
                                        <td class="px-4 py-3 border text-center bg-blue-50">
                                            <span class="current-value font-medium text-blue-600 text-lg"
                                                ...
                                                @if ($currentQuantity > 0)
                                                    {{ number_format($currentExtractValue, 2) }}
                            ...
                            <tfoot class="bg-gray-100 font-bold">
                                <tr>
                                    ...
                                    <td class="px-4 py-3 border text-center font-bold text-blue-600"
                                        ...
                                        {{ number_format($totalCurrentExtract, 2) }}
                                    </td>
                                </tr>
                            </tfoot>
                        </table>
                    ...
                    <div class="p-6 bg-gray-50 border-t">
                            ...
                            <div class="space-y-4">
                                ...
                                <div class="space-y-2">
                                    ...
                                    <div class="flex justify-between border-t pt-2">
                                        ...
                                        <span class="font-bold text-blue-600 text-lg" id="summary-current-extract">{{ number_format($totalCurrentExtract, 2) }}
                                            ر.س</span>
                                    </div>
                                    <div class="flex justify-between">
                            ...
                            <div class="space-y-4">
                                ...
                                <div class="space-y-3">
                                    @php
                                        ...
                                        $currentPercentage = $project->budget > 0 ? ($totalCurrentExtract / $project->budget) * 100 : 0;
                                    ...
                                    <div>
                                        <div class="flex justify-between mb-1">
                                            ...
                                            <span class="text-sm font-medium" id="current-percentage">{{ number_format($currentPercentage, 1) }}%</span>
                                        </div>
                                        <div class="w-full bg-gray-200 rounded-full h-2">
                        ...
                        <div class="mt-6 p-4 bg-white rounded-lg border">
                            ...
                            <p class="text-gray-800" id="amount-in-words">
                                @if($totalCurrentExtract > 0)
                                    {{ $totalCurrentExtract > 0 ? 'تم حساب المبلغ بالحروف' : 'صفر ريال سعودي' }}
                                @else
                                    صفر ريال سعودي
                                @endif
                            </p>
                        ...
                        <div class="mt-6 p-6 bg-gray-50 rounded-lg border">
                                ...
                                <div class="space-y-4">
                                    <div>
                                        ...
                                        <input type="text" id="extract_number" name="extract_number"
                                            value="{{ old('extract_number', $extract->extract_number) }}"
                                    ...
                                    <div>
                                        ...
                                        <textarea id="extract_description" name="extract_description" rows="3"
                                            class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                                            placeholder="وصف تفصيلي للمستخلص">{{ old('extract_description', $extract->description) }}</textarea>
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <input type="date" id="extract_date" name="extract_date"
                                            value="{{ old('extract_date', $extract->extract_date->format('Y-m-d')) }}"
                                            class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                                            required>
                                    </div>
                                </div>
                                ...
                                <div class="space-y-4">
                                    ...
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700 mb-2">
                                            رفع ملف المستخلص (اختياري)
                                        </label>
                                        <div class="border-2 border-dashed border-gray-300 rounded-lg p-4 text-center hover:border-gray-400 transition-colors"
                                            id="file-upload-area">
                                            <input type="file" id="extract_file" name="extract_file" class="hidden"
                                                accept=".pdf,.doc,.docx,.xls,.xlsx,.jpg,.jpeg,.png"
                                                onchange="handleFileSelect(this)">
                                            <div class="cursor-pointer" onclick="document.getElementById('extract_file').click()">
                                        ...
                                        @if($extract->file_path)
                                            <div class="mt-2 p-3 bg-green-50 rounded-lg">
                                                <div class="flex items-center gap-2">
                                                    <i class="ri-file-line text-green-600"></i>
                                                    <span class="text-sm font-medium text-green-900">الملف الحالي: {{ basename($extract->file_path) }}</span>
                                                    <a href="{{ Storage::url($extract->file_path) }}" target="_blank"
                                                        class="text-green-600 hover:text-green-800 ml-auto">
                                                        <i class="ri-download-line"></i>
                                                        تحميل
                                                    </a>
                                                </div>
                                                <p class="text-xs text-green-600 mt-1">رفع ملف جديد سيستبدل الملف الحالي</p>
                                            </div>
                                        @endif
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <select id="extract_status" name="extract_status"
                                            class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                                            <option value="draft" {{ old('extract_status', $extract->status) === 'draft' ? 'selected' : '' }}>مسودة</option>
                                            <option value="submitted" {{ old('extract_status', $extract->status) === 'submitted' ? 'selected' : '' }}>مُقدم</option>
                                            <option value="approved" {{ old('extract_status', $extract->status) === 'approved' ? 'selected' : '' }}>موافق عليه</option>
                                            <option value="paid" {{ old('extract_status', $extract->status) === 'paid' ? 'selected' : '' }}>مدفوع</option>
                                        </select>
                                    </div>
                                </div>
                            </div>
                            ...
                            <input type="hidden" id="extract_total" name="extract_total" value="{{ $totalCurrentExtract }}">
                            ...
                            <div class="flex items-center justify-between mt-6 pt-6 border-t border-gray-200">
                                <div class="flex items-center gap-2 text-sm text-gray-600">
                                    ...
                                    <span>سيتم حفظ جميع التعديلات</span>
                                </div>
                                <div class="flex items-center gap-3">
                                    <a href="{{ route('projects.extract.show', [$project, $extract]) }}"
    ...
    <script>
        // Initialize the form with current data on page load
        ...
        function updateProgressBars(currentExtract, projectBudget, previousPaid) {
            ...
            document.getElementById('total-percentage').textContent = Math.min(totalPercentage, 100).toFixed(1) + '%';

            // تحديث أشرطة التقدم
            document.getElementById('current-progress-bar').style.width = Math.min(currentPercentage, 100) + '%';
