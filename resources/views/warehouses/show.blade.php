    ...
    <script>
        ...
        window.printPartBarcode = function(code, name) {
            try {
                ...
                const scriptContent = 'window.addEventListener("load",function(){try{document.getElementById("partName").textContent="' + name.replace(/"/g, '\\"') + '";document.getElementById("partCode").textContent="' + code.replace(/"/g, '\\"') + '";document.getElementById("infoCode").textContent="' + code.replace(/"/g, '\\"') + '";document.getElementById("infoName").textContent="' + name.replace(/"/g, '\\"') + '";document.getElementById("infoDate").textContent="' + currentDate.replace(/"/g, '\\"') + '";var qrData="' + code + '|' + name + '";console.log("Creating QR Code with data:",qrData);var qr=new QRCode(document.getElementById("qrcode"),{text:qrData,width:300,height:300,colorDark:"#000000",colorLight:"#FFFFFF",correctLevel:QRCode.CorrectLevel.L});console.log("QR Code created successfully");setTimeout(function(){window.print();setTimeout(function(){window.close();},2000);},500);}catch(e){console.error("Error creating QR Code:",e);document.getElementById("qrcode").innerHTML="<p style=\\"color:red;\\">Error: "+e.message+"</p>";}});';
