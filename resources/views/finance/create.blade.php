...
<div class="p-6" dir="rtl">
    ...
    <div class="flex items-center gap-4 mb-6">
        ...
        <div>
            <h1 class="text-2xl font-bold text-gray-900">إضافة معاملة مالية</h1>
            <p class="text-gray-600 mt-1">إدخال معاملة مالية جديدة</p>
        </div>
    </div>
    ...
    <div class="max-w-4xl">
        <div class="bg-white rounded-xl shadow-sm border">
            ...
            <form action="{{ route('finance.store') }}" method="POST" class="p-6 space-y-6">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="category"
                            ...
                            <!-- Income Categories -->
                            <optgroup label="فئات الإيرادات" id="income-categories" style="display: none;">
                                <option value="مشاريع البناء" {{ old('category') == 'مشاريع البناء' ? 'selected' : '' }}>مشاريع البناء</option>
                                <option value="استشارات هندسية" {{ old('category') == 'استشارات هندسية' ? 'selected' : '' }}>استشارات هندسية</option>
                                <option value="مبيعات المواد" {{ old('category') == 'مبيعات المواد' ? 'selected' : '' }}>مبيعات المواد</option>
                                <option value="إيجارات" {{ old('category') == 'إيجارات' ? 'selected' : '' }}>إيجارات</option>
                                <option value="أخرى" {{ old('category') == 'أخرى' ? 'selected' : '' }}>أخرى</option>
                            </optgroup>
                            <!-- Expense Categories -->
                            <optgroup label="فئات المصروفات" id="expense-categories" style="display: none;">
                                <option value="المواد والمعدات" {{ old('category') == 'المواد والمعدات' ? 'selected' : '' }}>المواد والمعدات</option>
                                <option value="الرواتب والأجور" {{ old('category') == 'الرواتب والأجور' ? 'selected' : '' }}>الرواتب والأجور</option>
                                <option value="النقل والمواصلات" {{ old('category') == 'النقل والمواصلات' ? 'selected' : '' }}>النقل والمواصلات</option>
                                <option value="صيانة وإصلاح" {{ old('category') == 'صيانة وإصلاح' ? 'selected' : '' }}>صيانة وإصلاح</option>
                                <option value="مصاريف إدارية" {{ old('category') == 'مصاريف إدارية' ? 'selected' : '' }}>مصاريف إدارية</option>
                                <option value="تأمين" {{ old('category') == 'تأمين' ? 'selected' : '' }}>تأمين</option>
                                <option value="ضرائب ورسوم" {{ old('category') == 'ضرائب ورسوم' ? 'selected' : '' }}>ضرائب ورسوم</option>
                                <option value="أخرى" {{ old('category') == 'أخرى' ? 'selected' : '' }}>أخرى</option>
                            </optgroup>
                        </select>
                        @error('category')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="payment_method"
                            ...
                            <option value="بطاقة مدين" {{ old('payment_method') == 'بطاقة مدين' ? 'selected' : '' }}>بطاقة مدين</option>
                        </select>
