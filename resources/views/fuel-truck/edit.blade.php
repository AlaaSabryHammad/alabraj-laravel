...
@section('title', 'تعديل بيانات سيارة المحروقات')
            ...
            <div class="bg-white rounded-2xl shadow-sm border">
                <div class="px-6 py-4 border-b border-gray-200 bg-gray-50 rounded-t-2xl">
                    <div class="flex items-center gap-3">
                        ...
                        <h2 class="text-lg font-semibold text-gray-900">تعديل بيانات السيارة</h2>
                    </div>
                </div>

                <form action="{{ route('fuel-truck.update', $equipment) }}" method="POST" class="p-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <select name="fuel_type" id="fuel_type"
                                ...
                                <option value="diesel" @selected($equipment->fuelTruck->fuel_type === 'diesel')>ديزل</option>
                                <option value="gasoline" @selected($equipment->fuelTruck->fuel_type === 'gasoline')>بنزين</option>
                                <option value="engine_oil" @selected($equipment->fuelTruck->fuel_type === 'engine_oil')>زيت ماكينة</option>
                                <option value="hydraulic_oil" @selected($equipment->fuelTruck->fuel_type === 'hydraulic_oil')>زيت هيدروليك</option>
                                <option value="radiator_water" @selected($equipment->fuelTruck->fuel_type === 'radiator_water')>ماء ردياتير</option>
                                <option value="brake_oil" @selected($equipment->fuelTruck->fuel_type === 'brake_oil')>زيت فرامل</option>
                                <option value="other" @selected($equipment->fuelTruck->fuel_type === 'other')>أخرى</option>
                            </select>
                            @error('fuel_type')
                        ...
                        <div>
                            ...
                            <input type="number" name="capacity" id="capacity" step="0.01" min="0.01"
                                ...
                                value="{{ old('capacity', $equipment->fuelTruck->capacity) }}" placeholder="مثال: 1000" required>
                            @error('capacity')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="number" name="current_quantity" id="current_quantity" step="0.01" min="0"
                                ...
                                value="{{ old('current_quantity', $equipment->fuelTruck->current_quantity) }}" placeholder="مثال: 500" required>
                        ...
                        <div>
                            ...
                            <textarea name="notes" id="notes" rows="4"
                                ...
                                placeholder="أضف أي ملاحظات إضافية...">{{ old('notes', $equipment->fuelTruck->notes) }}</textarea>
                            @error('notes')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        </div>
