    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        <div class="p-4 border-b border-gray-100">
                    ...
                    <div id="filtersContainer"
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
                            ...
                            <div>
                                ...
                                <select name="department" class="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" onchange="submitFilters()">
                            ...
                            <div>
                                ...
                                <select name="role" class="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" onchange="submitFilters()">
                            ...
                            <div>
                                ...
                                <select name="status" class="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" onchange="submitFilters()">
                            ...
                            <div>
                                ...
                                <select name="location_id" class="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" onchange="submitFilters()">
                            ...
                            <div>
                                ...
                                <select name="has_user" class="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" onchange="submitFilters()">
                                    <option value="">الكل</option>
                                    <option value="1" {{ request('has_user') == '1' ? 'selected' : '' }}>متصل</option>
                                    <option value="0" {{ request('has_user') == '0' ? 'selected' : '' }}>غير متصل</option>
                                </select>
                            </div>
                        </div>
                    ...
                    <div class="mt-4 flex flex-wrap gap-2">
                        ...
                        @if(request('department'))
                            <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                                ...
                                <a href="{{ request()->fullUrlWithQuery(['department' => null]) }}" class="mr-1 text-blue-600 hover:text-blue-800">
                                    <i class="ri-close-line"></i>
                                </a>
                            </span>
                        @endif
                        ...
                        @if(request('role'))
                            <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-green-100 text-green-800">
                                الدور: {{ request('role') }}
                                <a href="{{ request()->fullUrlWithQuery(['role' => null]) }}" class="mr-1 text-green-600 hover:text-green-800">
                                    <i class="ri-close-line"></i>
                                </a>
                            </span>
                        @endif
                        ...
                        @if(request('location_id'))
                            ...
                            @if($selectedLocation)
                                <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-purple-100 text-purple-800">
                                    ...
                                    <a href="{{ request()->fullUrlWithQuery(['location_id' => null]) }}" class="mr-1 text-purple-600 hover:text-purple-800">
                                        <i class="ri-close-line"></i>
                                    </a>
                                </span>
                            @endif
                        ...
                        @if(request('has_user'))
                            <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-teal-100 text-teal-800">
                                ...
                                <a href="{{ request()->fullUrlWithQuery(['has_user' => null]) }}" class="mr-1 text-teal-600 hover:text-teal-800">
                                    <i class="ri-close-line"></i>
                                </a>
                            </span>
                        @endif
                        ...
                        @if(request('hire_date_from') || request('hire_date_to'))
                            <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-indigo-100 text-indigo-800">
                                ...
                                @endif
                                <a href="{{ request()->fullUrlWithQuery(['hire_date_from' => null, 'hire_date_to' => null]) }}" class="mr-1 text-indigo-600 hover:text-indigo-800">
                                    <i class="ri-close-line"></i>
                                </a>
                            </span>
                        @endif
            ...
            @if(request('search'))
                <div class="mt-3 text-sm text-gray-600">
                    ...
                    <span class="mr-2">({{ $employees->total() }} {{ $employees->total() == 1 ? 'نتيجة' : 'نتائج' }})</span>
                </div>
            @endif
        </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full min-w-max">
                <thead class="bg-gray-50">
                    <tr>
                        <th class="px-3 py-2 text-right text-xs font-medium text-gray-500 uppercase tracking-wider min-w-[200px]">الموظف</th>
                        <th class="px-3 py-2 text-right text-xs font-medium text-gray-500 uppercase tracking-wider min-w-[120px] hidden md:table-cell">المنصب</th>
                        <th class="px-3 py-2 text-right text-xs font-medium text-gray-500 uppercase tracking-wider min-w-[100px] hidden lg:table-cell">القسم</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-3 py-3 whitespace-nowrap text-sm text-gray-900 hidden md:table-cell">{{ $employee->position }}</td>
                        <td class="px-3 py-3 whitespace-nowrap text-sm text-gray-900 hidden lg:table-cell">{{ $employee->department }}</td>
                        <td class="px-3 py-3 whitespace-nowrap">
        ...
        <div class="text-center py-12">
            @if(request('search'))
                ...
                <p class="text-gray-500 mb-6">لم يتم العثور على موظفين مطابقين لكلمة البحث "{{ request('search') }}"</p>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        const hasActiveFilters = {{ request()->hasAny(['department', 'role', 'status', 'location_id', 'has_user', 'hire_date_from', 'hire_date_to']) ? 'true' : 'false' }};
        if (hasActiveFilters) {
            const container = document.getElementById('filtersContainer');
            const icon = document.getElementById('filterIcon');
            container.classList.remove('hidden');
