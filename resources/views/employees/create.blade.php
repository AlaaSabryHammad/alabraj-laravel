        ...
        <form action="{{ route('employees.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div>
                ...
                <div class="mb-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
                    <div class="flex items-start gap-2">
                        ...
                        <div class="text-xs text-yellow-800">
                            <strong>تنبيه:</strong> معلومات الوظيفة ستؤثر على صلاحيات الموظف في النظام. تأكد من اختيار المنصب والقسم والصلاحية بدقة.
                        </div>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        <label for="working_hours" class="block text-sm font-medium text-gray-700 mb-2">عدد ساعات العمل اليومية *</label>
                        <input type="number"
                               id="working_hours"
                               name="working_hours"
                               value="{{ old('working_hours', 8) }}"
                               min="1"
                               max="24"
                               step="0.5"
            ...
            <div>
                ...
                <div class="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                    <div class="flex items-start gap-2">
                        ...
                        <div class="text-xs text-blue-800">
                            ...
                            <ul class="list-disc pl-4 mt-1 space-y-1">
                                <li><strong>هذا القسم اختياري تماماً</strong> - يمكنك تخطيه والعودة لإضافة الوثائق لاحقاً</li>
                ...
                <div class="bg-gray-50 rounded-xl p-6">
                        ...
                        <div class="document-row bg-white p-4 rounded-lg border border-gray-200">
                            <div class="flex items-center justify-between mb-3">
                                ...
                                <div class="flex items-center gap-2">
                                    ...
                                    <button type="button" onclick="removeDocumentRow(this)" class="text-red-600 hover:text-red-800 text-xs">
                    ...
                    <div class="mt-4 text-center">
                        ...
                        <p class="text-xs text-gray-500 mt-2">يمكنك تخطي هذا القسم بالكامل إذا لم تكن تريد إضافة وثائق إضافية</p>
                    </div>
                </div>
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const requiredFields = [
        { id: 'name', name: 'الاسم الكامل' },
        { id: 'national_id', name: 'رقم الهوية' },
        { id: 'email', name: 'البريد الإلكتروني' },
        { id: 'phone', name: 'رقم الهاتف' },
        { id: 'nationality', name: 'الجنسية' },
        { id: 'position', name: 'المنصب' },
        { id: 'department', name: 'القسم' },
        { id: 'hire_date', name: 'تاريخ التوظيف' },
        { id: 'salary', name: 'الراتب' },
        { id: 'location_id', name: 'الموقع' },
    ...
    if (form) {
        form.addEventListener('submit', function(e) {
            ...
            if (nationalIdField && nationalIdField.value.trim() && !validateSaudiId(nationalIdField.value.trim())) {
                hasErrors = true;
                nationalIdField.classList.add('border-red-500');
                showValidationError('رقم الهوية غير صحيح (يجب أن يبدأ بـ 1 أو 2 ويحتوي على 10 أرقام)');
            }
...
function showDataPreview() {
    ...
    // فحص الحقول المطلوبة
    const requiredFields = [
        { field: 'name', name: 'الاسم الكامل' },
        { field: 'national_id', name: 'رقم الهوية' },
        { field: 'email', name: 'البريد الإلكتروني' },
        { field: 'phone', name: 'رقم الهاتف' },
        { field: 'position', name: 'المنصب' },
        { field: 'department', name: 'القسم' },
        { field: 'salary', name: 'الراتب' }
    ];
    ...
    warnings.forEach(warning => {
        const warningClass = warning.type === 'error' ? 'border-red-400 bg-red-50 text-red-800' : 'border-yellow-400 bg-yellow-50 text-yellow-800';
        const iconClass = warning.type === 'error' ? 'ri-error-warning-line text-red-600' : 'ri-alert-line text-yellow-600';
