...
@section('content')
    <div class="p-6" dir="rtl">
        ...
        <div class="flex items-center justify-between mb-6 no-print">
            ...
            <div class="flex items-center gap-3">
                ...
                {{-- عرض زر تعيين مدير مباشر فقط للأدوار التي ليست في أعلى التسلسل --}}
                @php
                    $requiredManagerRole = \App\Models\Employee::MANAGER_CHAIN[$employee->role] ?? null;
                @endphp
                    ...
                    @if ($employee->directManager)
                        <div class="bg-blue-50 p-3 rounded-lg border border-blue-200">
                            ...
                            <div class="flex items-center space-x-2 space-x-reverse">
                                ...
                                <div>
                                    <p class="text-sm font-medium text-blue-900">{{ $employee->directManager->name }}</p>
                ...
                <div class="p-6 border-b border-gray-200 employee-section">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        <div class="bg-green-50 p-3 rounded-lg">
                            <label class="block text-xs font-medium text-green-600 mb-1">البريد الإلكتروني للدخول</label>
                    ...
                    <div class="mt-4 bg-blue-50 border border-blue-200 rounded-lg p-4">
                        ...
                        <div class="space-y-2 text-sm text-blue-700">
                            ...
                            <p class="text-xs"><em>يرجى إبلاغ الموظف بهذه المعلومات لتمكينه من الدخول إلى النظام</em></p>
                        </div>
                    </div>
                </div>
            @else
                        ...
                        @if ($employee->national_id_photo)
                            <div class="mb-3">
                                <img src="{{ asset('storage/' . $employee->national_id_photo) }}"
                                    alt="صورة الهوية الوطنية" class="w-full h-32 object-contain rounded border bg-white">
                            </div>
                        ...
                        @if ($employee->work_permit_photo)
                            <div class="mb-3">
                                <img src="{{ asset('storage/' . $employee->work_permit_photo) }}"
                                    alt="صورة بطاقة التشغيل" class="w-full h-32 object-contain rounded border bg-white">
                            </div>
                        @else
                            <div class="w-full h-32 bg-gray-100 rounded border flex items-center justify-center mb-3">
                                <div class="text-center">
                        ...
                        @if ($employee->driving_license_photo)
                            <div class="mb-3">
                                <img src="{{ asset('storage/' . $employee->driving_license_photo) }}"
                                    alt="صورة رخصة القيادة" class="w-full h-32 object-contain rounded border bg-white">
                            </div>
                        @else
                            <div class="w-full h-32 bg-gray-100 rounded border flex items-center justify-center mb-3">
                                <div class="text-center">
                        ...
                        @foreach ($employee->drivenEquipment as $equipment)
                            <div
                                ...
                                <div class="flex items-center gap-4">
                                    ...
                                    <!-- Equipment Details -->
                                    <div class="flex-1">
                                        <h4 class="text-lg font-semibold text-gray-900 mb-2">{{ $equipment->name }}</h4>

                                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 text-sm">
                                            <div class="flex items-center gap-2">
                                                <span class="font-medium text-gray-700">النوع:</span>
                                                <span class="text-gray-600">{{ $equipment->type ?? 'غير محدد' }}</span>
                                            </div>
                    ...
                    <div class="mt-6 bg-gray-50 rounded-lg p-4">
                        ...
                        <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 text-center">
                            <div class="bg-white rounded-lg p-3 border">
                                <div class="text-2xl font-bold text-gray-900">{{ $employee->drivenEquipment->count() }}
                                </div>
    ...
    <!-- Activation Modal -->
    @if (auth()->user() && auth()->user()->isGeneralManager() && $employee->status === 'inactive')
        <div id="activationModal"
            ...
            <div class="relative top-20 mx-auto p-5 border w-11/12 md:w-1/2 lg:w-1/3 shadow-lg rounded-lg bg-white"
                ...
                <div class="flex items-center justify-between pb-4 border-b">
                    <h3 class="text-xl font-bold text-gray-900">تفعيل الموظف</h3>
                ...
                <div class="py-6">
                    <div class="flex items-center mb-4">
                        <div class="flex-shrink-0">
                            <div class="flex items-center justify-center h-12 w-12 rounded-full bg-emerald-100">
                                <i class="ri-user-follow-line text-2xl text-emerald-600"></i>
                            </div>
                        </div>
                        <div class="mr-4">
                            <h4 class="text-lg font-medium text-gray-900">{{ $employee->name }}</h4>
                            <p class="text-sm text-gray-600">{{ $employee->position }} - {{ $employee->department }}
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-4">
                        <div class="flex items-start">
                            <i class="ri-alert-line text-yellow-600 text-xl ml-3 mt-0.5"></i>
                            <div>
                                <h5 class="font-medium text-yellow-800">تنبيه هام</h5>
                                <p class="text-yellow-700 text-sm mt-1">
                                    بتفعيل هذا الموظف، سوف يتمكن من:
                                </p>
                                <ul class="text-yellow-700 text-sm mt-2 mr-4">
                                    <li>• الدخول إلى النظام</li>
                                    <li>• الوصول إلى بياناته الشخصية</li>
                                    <li>• استخدام جميع ميزات النظام المخصصة له</li>
                                </ul>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-emerald-50 border border-emerald-200 rounded-lg p-4">
                        <div class="flex items-center">
                            <i class="ri-check-circle-line text-emerald-600 text-xl ml-3"></i>
                ...
                <div class="flex justify-end gap-3 pt-4 border-t">
                    ...
                    <form method="POST" action="{{ route('employees.activate', $employee) }}" class="inline">
                        @csrf
                        @method('PATCH')
                        <button type="submit"
                            class="px-6 py-2 text-sm font-medium text-white bg-emerald-600 border border-transparent rounded-lg hover:bg-emerald-700 transition-colors">
                            تفعيل الموظف
                        </button>
                    </form>
                </div>
    ...
    <!-- Deactivation Modal -->
    @if (auth()->user() && auth()->user()->isGeneralManager() && $employee->status === 'active')
        <div id="deactivationModal"
            class="fixed inset-0 bg-gray-600 bg-opacity-50 hidden overflow-y-auto h-full w-full z-50">
            <div class="relative top-20 mx-auto p-5 border w-11/12 md:w-1/2 lg:w-1/3 shadow-lg rounded-lg bg-white"
                dir="rtl">
                <!-- Modal Header -->
                <div class="flex items-center justify-between pb-4 border-b">
                    <h3 class="text-xl font-bold text-gray-900">إلغاء تفعيل الموظف</h3>
                ...
                <div class="py-6">
                    <div class="flex items-center mb-4">
                        <div class="flex-shrink-0">
                            <div class="flex items-center justify-center h-12 w-12 rounded-full bg-amber-100">
                                <i class="ri-user-unfollow-line text-2xl text-amber-600"></i>
                            </div>
                        </div>
                        <div class="mr-4">
                            <h4 class="text-lg font-medium text-gray-900">{{ $employee->name }}</h4>
                    ...
                    <div class="bg-red-50 border border-red-200 rounded-lg p-4 mb-4">
                        <div class="flex items-start">
                            <i class="ri-alert-line text-red-600 text-xl ml-3 mt-0.5"></i>
                            <div>
                                <h5 class="font-medium text-red-800">تحذير هام</h5>
                                <p class="text-red-700 text-sm mt-1">
                                    بإلغاء تفعيل هذا الموظف، سوف يتم:
                                </p>
                                <ul class="text-red-700 text-sm mt-2 mr-4">
                                    <li>• منعه من الدخول إلى النظام</li>
                                    <li>• إيقاف جميع صلاحياته</li>
                                    <li>• عدم قدرته على استخدام التطبيق</li>
                                    <li>• إزالة الوصول لجميع البيانات</li>
                                </ul>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-amber-50 border border-amber-200 rounded-lg p-4">
                        <div class="flex items-center">
                            <i class="ri-error-warning-line text-amber-600 text-xl ml-3"></i>
                            <p class="text-amber-800 text-sm">
                                هل أنت متأكد من إلغاء تفعيل هذا الموظف؟ يمكنك إعادة تفعيله لاحقاً.
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex justify-end gap-3 pt-4 border-t">
                    ...
                    <form method="POST" action="{{ route('employees.deactivate', $employee) }}" class="inline">
                        @csrf
                        @method('PATCH')
                        <button type="submit"
                            class="px-6 py-2 text-sm font-medium text-white bg-amber-600 border border-transparent rounded-lg hover:bg-amber-700 transition-colors">
                            إلغاء تفعيل الموظف
                        </button>
                    </form>
                </div>
            ...
            <form action="{{ route('employees.assign-manager', $employee) }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    @if ($requiredRole)
                        <div class="mb-3 p-3 bg-blue-50 rounded-lg border border-blue-200">
                            <p class="text-sm text-blue-700">
                                ...
                                يجب أن يكون المدير المباشر من الدور: <strong>{{ $requiredRole }}</strong>
                            </p>
                        </div>
                        @if ($employee->role === 'عامل')
                            <div class="mb-3 p-3 bg-yellow-50 rounded-lg border border-yellow-200">
                                <p class="text-sm text-yellow-700">
                                    ...
                                    <strong>تنبيه:</strong> عند تعيين مدير مباشر، سيتم تحديث موقع العمل للموظف تلقائياً
                                    ليصبح
                                    نفس موقع عمل المدير المباشر
                                </p>
                            </div>
                        @endif
                    @else
                        <div class="mb-3 p-3 bg-purple-50 rounded-lg border border-purple-200">
                            <p class="text-sm text-purple-700">
                                <i class="ri-information-line ml-1"></i>
                                هذا الدور في أعلى التسلسل ولا يمكن تعيين مدير أعلى.
                            </p>
                        </div>
                    @endif
                    <select name="direct_manager_id" id="direct_manager_id"
                        ...
                        @if ($requiredRole)
                            @foreach (\App\Models\Employee::where('role', $requiredRole)->where('id', '!=', $employee->id)->orderBy('name')->get() as $manager)
                            ...
                            {{ $assignment->assignment_type === 'تعيين' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                ...
                                <td class="px-4 py-3">
                                    @if ($assignment->manager)
                                        <div class="flex items-center gap-2">
                                            ...
                                            @endif
                                            <div>
                                                <div class="text-gray-900 font-medium">{{ $assignment->manager->name }}
                                                </div>
                                                <div class="text-xs text-gray-500">{{ $assignment->manager->position }}
                                                </div>
                                            </div>
                                        </div>
                                    @else
                                ...
                                <td class="px-4 py-3">
                                    @if ($assignment->assignedBy)
                                        <div class="text-gray-900 font-medium">{{ $assignment->assignedBy->name }}</div>
    ...
    <script>
        // دوال تفعيل وإلغاء تفعيل الموظف
