...
<div class="space-y-6">
    ...
    <!-- Filters and Search -->
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
        <form method="GET" action="{{ route('external-trucks.index') }}" class="flex flex-wrap items-end gap-4" id="filterForm">
            <!-- Search -->
            ...
            <div class="min-w-40">
                ...
                <select name="status" class="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500" onchange="this.form.submit()">
            ...
            <div class="min-w-40">
                ...
                <select name="loading_type" class="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500" onchange="this.form.submit()">
                    <option value="">جميع الأنواع</option>
                    <option value="box" {{ request('loading_type') == 'box' ? 'selected' : '' }}>صندوق</option>
                    <option value="tank" {{ request('loading_type') == 'tank' ? 'selected' : '' }}>تانك</option>
                </select>
            </div>
            ...
            <div class="flex space-x-2 space-x-reverse">
                ...
                @if(request()->hasAny(['search', 'status', 'loading_type']))
        ...
        <div class="overflow-x-auto">
            <table class="w-full">
                <thead class="bg-gray-50">
                    <tr>
                        <th class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">الشاحنة</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50">
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex items-center">
                                ...
                                <div class="mr-4">
                                    ...
                                    <div class="text-sm text-gray-500">{{ $truck->loading_type_text }}</div>
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $truck->loading_type === 'box' ? 'bg-blue-100 text-blue-800' : 'bg-purple-100 text-purple-800' }}">
                                {{ $truck->loading_type_text }}
                            </span>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $truck->capacity_with_unit }}</td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            @php
                                $statusColors = [
                            ...
                            @endphp
                            <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $statusColors[$truck->status] ?? 'bg-gray-100 text-gray-800' }}">
                                {{ $truck->status_text }}
                            </span>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                            <div class="flex space-x-2 space-x-reverse">
                                ...
                                <form action="{{ route('external-trucks.destroy', $truck) }}"
                                      method="POST"
